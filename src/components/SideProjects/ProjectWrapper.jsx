...
const ProjectWrapper = React.forwardRef((props, ref) => {
  ...
  const setPosition = (event) => {
    ...
    controls.start({
      rotateX: (offsetY / height) * -20,
      rotateY: (offsetX / width) * 20,
      transition: { duration: 0.6, ease: [0.23, 1, 0.32, 1] },
    });
    bgControls.start({
      translateX: (offsetX / width) * -35,
      translateY: (offsetY / height) * -35,
      opacity: 0.8,
      transition: { duration: 0.6, ease: [0.23, 1, 0.32, 1] },
    });
  ...
  return (
    ...
    >
      ...
      >
        <Heading as="h3" size="lg" mb="1" zIndex="1">
          {props.title}
        </Heading>
        <Text textStyle="paragraph-sm" maxW="40ch" zIndex="1">
          {props.info}
        </Text>
        <Link
          className="view-live"
